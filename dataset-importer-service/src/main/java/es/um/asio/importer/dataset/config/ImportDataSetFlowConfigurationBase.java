...
public abstract class ImportDataSetFlowConfigurationBase {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ImportDataSetFlowConfigurationBase.class);

	
	@Autowired
	private ImportProperties importDataSetProperties;
	...
	protected <T extends DataSetData> Step createStep(Class<T> type, String filePath) {
		return this.stepBuilderFactory.get(type.getSimpleName().concat("-").concat(UUID.randomUUID().toString()))
				.<DataSetData, InputData<DataSetData>>chunk(1000).reader(baseReader(type, filePath))
	...
	/**
	 * Gets the {@link ItemReader} implementation that reads the XML file and
	 * transform to {@link type}
	 *
	 * @param <T>      the generic type
	 * @param type     the type
	...
	private Resource[] getRelatedResources(final String path) {
		boolean isClassPathResource = StringUtils.isBlank(this.importDataSetProperties.getDataSetPath());
		String filePath = isClassPathResource ? path
				: new File(this.importDataSetProperties.getDataSetPath(), path).getPath();
